...
best = [0, 0, 0] #acuracia, melhor max depth, melhor num estimadores
...
for depth in depths:
    for nclass in n_classificadores:
        rfc = RandomForestClassifier(max_depth=depth, n_estimators=nclass)
        print(f"### Max Depth={depth} Estimadores={nclass} ###")
        ...
        if best[0] <= f1_score_validacao:
            best = [f1_score_validacao, depth, nclass]
        print("#############################")    
print(f"Melhor F1-Score de Validação: {best[0]}, Max Depth={best[1]}, Estimadores={best[2]}")

# Rodada completa com melhor gamma e C
print("Rodada completa de Treinamento com melhor Max Depth e Estimadores")
rfc_treino_total = RandomForestClassifier(max_depth=best[1], n_estimators=best[2])

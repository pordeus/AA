...
best = [0, 0, 0] #acuracia, melhor gamma, melhor C
...
for Ci in Cs:
    for gam in gammas:
        ...
        print(f"### Gamma={2**gam} C={Ci} ###")

        media_acuracia_val, media_acuracia_train, media_precisao_treino, media_revocacao_treino, media_precisao_val, media_revocacao_val = avalia_classificador_mais_metricas(svc, kf, X_treino, Y_treino, accuracy_score) 
        ...
        if best[0] <= f1_score_validacao:
            best = [f1_score_validacao, gam, Ci]
        print("#############################")    
print(f"Melhor F1-Score de Validação: {best[0]}, Gamma={best[1]}, C={best[2]}")

# Rodada completa com melhor gamma e C
print("Rodada completa de Treinamento com melhor gamma e C")
scv_treino_total = svm.SVC(gamma=2**best[1], C=2**best[2])
metrica, precisao, revocacao, y_pred_final = rodadaUnica(scv_treino_total, X_treino, Y_treino, accuracy_score)
...
print("")
print("Treino")
metrica, precisao, revocacao, y_pred = rodadaUnica(scv_treino_total, X_teste, Y_teste, accuracy_score)
metrics.plot_roc_curve(scv_treino_total, X_teste, Y_teste)
disp = plot_precision_recall_curve(scv_treino_total, X_teste, Y_teste)
